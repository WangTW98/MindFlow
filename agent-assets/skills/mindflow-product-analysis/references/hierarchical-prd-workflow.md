...
Workflow version 3 is the default for new canvas-producing tasks. Workflow version 2 remains readable and resumable. Use this order without overlap:
...
4. Freeze `prd/page-index.json`, then write one PRD for each indexed `page`, `popup`, or independent business state. Version 3 page PRDs include an ordered visual-composition table whose rows map to analysis regions and features. Layouts, navigation, and shared components belong to the comprehensive PRD or their owning page unless they require independent canvas nodes.
...
Each page PRD records purpose, users and permissions, entry and exit conditions, ordered regions and UI elements, data inputs/outputs, applicable visible states and exceptions, outgoing navigation/data/status events, acceptance criteria, original evidence references, and unresolved decisions. Do not substitute generic inherited prose for page-specific composition or recovery behavior. The page owning the source feature outlet owns the edge; targets never duplicate it. Because skeleton and navigation nodes have no page PRDs, the target page PRD owns its one application-navigation entry edge and records the navigation outlet as external framework evidence.
