...
import type { EdgeType, FlowEdge, PageAction, PageElement, PageNode, ProductDesignIssue, ProductFlow } from "../models/productFlow";
import {
  makeActionId,
  makeChangeSetId,
  makeEdgeId,
...
export function createMockFlow(documentText: string, documentName = "示例需求", sourceDocumentId?: string): ProductFlow {
  ...
  return {
    ...
    productDesignIssues: createProductDesignIssues({
      supplierHomeNode,
      quoteNode,
      compareNode,
      inquiryNode,
...
function createProductDesignIssues(nodes: {
  supplierHomeNode: PageNode;
  quoteNode: PageNode;
  compareNode: PageNode;
  inquiryNode: PageNode;
  mobileDetailNode: PageNode;
  registerNode: PageNode;
  supplierReviewNode: PageNode;
}): ProductDesignIssue[] {
  return [
    {
      issueId: makeProductDesignIssueId("critical", "报价撤回缺少闭环路径"),
      severity: "critical",
      title: "报价撤回缺少闭环路径",
      description: "供应商门户描述了报价撤回场景，但当前流程只表现提交报价到报价对比，缺少撤回后的状态回滚、重新报价入口和采购侧通知。",
      prompt: "请基于当前 MindFlow 完善“报价撤回缺少闭环路径”：在供应商门户首页、报价填写页、报价对比页之间补充撤回报价后的状态、通知、重新报价入口和采购侧处理路径，并保持节点与边的 domainIds、roleIds、appSurfaceIds 一致。",
      relatedNodeIds: [nodes.supplierHomeNode.nodeId, nodes.quoteNode.nodeId, nodes.compareNode.nodeId],
      sourceRefs: [{ sourceId: "mock", label: "MockProvider", excerpt: "供应商查看询价、提交报价和撤回报价。" }]
    },
    {
      issueId: makeProductDesignIssueId("warning", "退回审批后的采购待办承接不清晰"),
      severity: "warning",
      title: "退回审批后的采购待办承接不清晰",
      description: "移动审批详情页可以退回修改并连到询价方案编辑页，但缺少采购专员收到退回待办、查看退回意见、修改后再次发起审批的明确闭环。",
      prompt: "请基于当前 MindFlow 完善“退回审批后的采购待办承接不清晰”：补充采购专员退回待办、退回意见查看、修改询价方案、重新生成比价报告并再次发起审批的节点或连线，明确每一步触发条件。",
      relatedNodeIds: [nodes.mobileDetailNode.nodeId, nodes.inquiryNode.nodeId, nodes.compareNode.nodeId],
      sourceRefs: [{ sourceId: "mock", label: "MockProvider", excerpt: "审批人查看采购摘要、报价对比、预算占用和风险提示并处理审批。" }]
    },
    {
      issueId: makeProductDesignIssueId("optional", "供应商注册后缺少进度反馈体验"),
      severity: "optional",
      title: "供应商注册后缺少进度反馈体验",
      description: "访客供应商提交注册后直接进入后台审核链路，当前流程未体现供应商侧的提交成功、审核进度查询和驳回后补充材料体验。",
      prompt: "请基于当前 MindFlow 优化“供应商注册后缺少进度反馈体验”：为公开采购网站或供应商门户补充注册提交成功、审核进度查询、驳回原因查看和补充材料路径，并标注对应角色与应用端。",
      relatedNodeIds: [nodes.registerNode.nodeId, nodes.supplierReviewNode.nodeId, nodes.supplierHomeNode.nodeId],
      sourceRefs: [{ sourceId: "mock", label: "MockProvider", excerpt: "潜在供应商提交企业信息、联系人和资质文件。" }]
    }
  ];
}

function makeProductDesignIssueId(severity: ProductDesignIssue["severity"], title: string): string {
  return `pdi_${severity}_${shortHash(title, 8)}`;
}
