...
6. For workflow-version 2 or 3 canvas tasks, finish the comprehensive product PRD and every indexed page PRD before graph design. Version 3 page PRDs must contain source-traceable visual composition. Validate them with `python3 scripts/validate_prd_bundle.py <task-dir>` and export the bundle before the first canvas write.
