...
2. Inventory every source before detailed analysis. Use `mindflow-from-documents`, `mindflow-from-code`, or `mindflow-from-canvas` according to source type; use several for mixed tasks.
...
5. Run `python3 scripts/validate_analysis_packet.py <analysis_packet.json>` before graph design or deliverable handoff.
...
- For canvas creation or update, hand the approved analysis packet to `mindflow-canvas-authoring`. Never send one full-canvas changeset.
